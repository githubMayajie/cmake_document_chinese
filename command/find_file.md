...
See policy CMP0074.
```<prefix>```/include/```<arch>``` if CMAKE_LIBRARY_ARCHITECTURE
...
```<prefix>```/include/```<arch>``` if CMAKE_LIBRARY_ARCHITECTURE
is set, and ```<prefix>```/include for each ```<prefix>``` in CMAKE_PREFIX_PATH
CMAKE_INCLUDE_PATH
...
```<entry>```/include for other entries in PATH, and the
